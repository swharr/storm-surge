...
check_script_requirements() {
    ...
    if [[ "$script" == *"/providers/"* ]]; then
        if grep -q "STORM_REGION" "$script" && grep -q "STORM_ZONE" "$script" && grep -q "STORM_NODES" "$script"; then
            echo "  ✅ $basename_script uses required environment variables"
        else
            echo "  ❌ $basename_script missing required environment variables (STORM_REGION, STORM_ZONE, STORM_NODES)"
            return 1
        fi
    fi
    ...
    if [[ "$basename_script" == "deploy.sh" ]]; then
        if grep -q "show_usage" "$script"; then
            echo "  ✅ $basename_script has usage function"
        else
            echo "  ❌ $basename_script missing usage function"
            return 1
        fi
    fi
}
...
test_parameter_validation() {
    ...
    if [[ "$basename_script" == "deploy.sh" ]]; then
        if timeout 5 "$script" --help > /dev/null 2>&1; then
            echo "  ❌ deploy.sh should exit with error for --help"
            return 1
        else
